...
class DatasourcePropsController {
    constructor($scope, reportDatasourceService) {
        ...
        $scope.onDataSourceChange = (dataSource: ReportDataSource) => {
            report.filterFields = [];
            if(report.dataSource) {
        ...
        $scope.onFilterFieldStatusChange = (field: ReportTableField) => {
            if(field.$checked) {
                let _field = $.extend({}, field);
                delete _field.$checked;
                report.filterFields.push(_field);
            } else {
                _.remove(report.filterFields, (f: ReportTableField) => {
